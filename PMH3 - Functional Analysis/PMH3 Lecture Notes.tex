...
% \setromanfont[Mapping=tex-text,Contextuals=w {NoWordInitial,NoWordFinal,NoLineInitial,NoLineFinal}]{Adobe Caslon Pro}
...
\title{PMH3 - Functional Analysis \\ Lecture Notes}								% Document Title
...
\begin{cor}
	Let $x \in \H$ and $S$ orthonormal.  Then \[
		\{ e \in S \, | \, \iprod{x, e} \neq 0 \}
	\] is countable.  
\end{cor}
...
The eigenvalues of an $n \times n$ matrix $T$ over $\C$ are the $\lambda \in \C$ with \[
	\det(\lambda I - T) = 0
...
\end{rem}
\begin{rem}
	We need our base field to be $\C$ to get reasonable spectral theory.
\end{rem}
...
\begin{thm}
	Let $X \neq \{ 0 \}$ be a Banach space over $\C$, and let $T \in \mathcal L(X)$.  Then $\sigma(T)$ is a non-empty, compact (closed and bounded) subset of \[
...
\begin{lem}
	With above assumptions $\sigma(T) \subseteq \{ \lambda \in \C \given | \lambda | \leq \| T \| \}$.  
\end{lem}
...
\begin{proof}
	Let \mapping{f}{\C}{\mathcal L(X)}{\lambda}{\lambda I - T}  This is continuous, as \begin{align*}
...
So $\sigma(T)$ is a compact subset of $\{ \lambda \in \C \given | \lambda | \leq \| T \| \}$.  Write $\rho(T) = \C \backslash \sigma(T)$ (the \textbf{resolvent set}), and let $R_T = R : \rho(T) \rightarrow \mathcal L (X)$ with $R_T(\lambda) = (\lambda I - T)^{-1}$.
...
\begin{proof}
	...
	Let $\phi = \mathcal L(X)'$ (hence $\phi : \mathcal L(X) \rightarrow C$.)  Let \mapping{f_\phi}{\rho(T)}{\C}{\lambda}{\phi(R(\lambda))} 
	\begin{lem}
	...
	Now suppose that $\sigma(T) = \emptyset$.  Then $f_\phi : \C \rightarrow \C$ is analytic.
...
Let $X \neq \{ 0 \}$ be a Banach space over $\C$.  The \textbf{spectral radius} of $T \in \mathcal L(X)$ is
\begin{align*}
...
\begin{note}
	\[
		r(T) \leq \| T \|
	\] since $\sigma(T) \subseteq \{ \lambda \in \C \given | \lambda | \leq \| T \| \}$.  Strict inequality can (and often does) occur.
\end{note}
...
\begin{exmp}
	Let \[
		T = \begin{pmatrix}
			...
			0 & 0 
		\end{pmatrix}.
	\]  Then consider $T : \C^2 \rightarrow \C^2$ where $\| (x, y) \|_2 = \sqrt{|x|^2 + |y|^2}$.  Then \begin{align*}
		\| T \| &= \sup \{ \| Tx \|_2 \given x \in \C^2 \} \\
...
\begin{thm}[Gelfand, 1941]
	Let $X \neq \{ 0 \}$ be Banach over $\C$, and let $T \in \mathcal L(X)$.  Then \begin{align*}
...
\begin{proof}
	...
	\]
	Let $\phi \in \mathcal L(X)$ and let \mapping{f_\phi}{\rho(T)}{\C}{\lambda}{\phi((\lambda I - T)^{-1})}
